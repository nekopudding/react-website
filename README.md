...
If user access to yarn is restricted from the current path, run `Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy Unrestricted`.

To set up dependencies, run `npm install`.

To test changes locally, use `yarn start` to start a local server.
